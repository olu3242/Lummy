    ...
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div className="h-40 w-40 flex-shrink-0">
          {mounted ? (
            <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={safeData} cx={75} cy={75} innerRadius={48} outerRadius={70} paddingAngle={3} dataKey="value" strokeWidth={0}>{safeData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}</Pie><Tooltip content={<CustomTooltip />} /></PieChart></ResponsiveContainer>
          ) : (
