      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0d0d1a] p-6 shadow-2xl">
        {step === "form" ? (
          ...
          <div className="text-center py-4">
            ...
            <p className="text-sm text-white/50 mt-1">Invitation sent to <strong className="text-white">{email}</strong> for <strong className="text-[#6C4EF3]">{role}</strong>.</p>
...
function ActionMenu({ member, onAction }: { member: OrgMember; onAction: (action: string, member: OrgMember) => void }) {
  const [open, setOpen] = useState(false)
  const actions =
    member.status === "active"
      ? [{ label: "Disable access", action: "disable", danger: false }, { label: "Suspend account", action: "suspend", danger: true }]
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} className="p-1.5 rounded-lg hover:bg-white/10 text-white/40 hover:text-white transition-colors">
...
export function TeamsClient({ initialMembers }: { initialMembers: OrgMember[] }) {
  ...
  function handleAction(action: string, member: OrgMember) {
    setMembers(prev => prev.map(m => {
      if (m.id !== member.id) return m
      if (action === "disable") return { ...m, status: "inactive" }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/10 bg-white/5">
              ...
              <th className="text-left py-3 px-4 text-white/50 font-medium text-xs">Joined</th>
          ...
          <tbody>
            {filtered.map(member => (
              <tr key={member.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                ...
                <td className="py-3 px-4 text-xs text-white/40">{new Date(member.joinedAt).toLocaleDateString()}</td>
                <td className="py-3 px-4">
