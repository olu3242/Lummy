...
const GUIDES: GuideSection[] = [
  {
    icon: Store, title: "Set up your store",
    color: "text-brand-purple", bg: "bg-brand-purple/10", href: "/dashboard/store",
  ...
  {
    icon: ShoppingBag, title: "Add your first product",
    color: "text-brand-green", bg: "bg-brand-green/10", href: "/dashboard/products",
  ...
  {
    icon: ClipboardList, title: "Manage orders",
    color: "text-brand-coral", bg: "bg-brand-coral/10", href: "/dashboard/orders",
  ...
  {
    icon: Megaphone, title: "Send a broadcast",
    color: "text-amber-500", bg: "bg-amber-500/10", href: "/dashboard/broadcast",
  ...
  {
    icon: Bot, title: "Use Lummy AI",
    color: "text-brand-indigo", bg: "bg-brand-indigo/10", href: "/dashboard/ai",
  ...
  {
    icon: BarChart3, title: "Read your analytics",
    color: "text-brand-purple", bg: "bg-brand-purple/10", href: "/dashboard/analytics",
...
const KEYBOARD_SHORTCUTS: { group: string; shortcuts: Shortcut[] }[] = [
  {
    ...
    shortcuts: [
      ...
      { keys: ["G", "C"], desc: "Go to Customers / CRM" },
  ...
  },
  {
    group: "Quick actions",
    shortcuts: [
      { keys: ["N", "P"], desc: "New product" },
      { keys: ["N", "O"], desc: "New order" },
      { keys: ["⌘", "K"], desc: "Open command palette" },
      { keys: ["⌘", "/"], desc: "Toggle search" },
    ],
  },
  {
    group: "General",
...
function ShortcutKey({ k }: { k: string }) {
  return (
    <kbd className="inline-flex items-center justify-center min-w-[22px] h-5.5 px-1.5 rounded-md border border-border bg-muted text-[10px] font-semibold font-mono shadow-sm">
      {k}
    </kbd>
  )
}
...
export default function HelpPage() {
  ...
  const handleTicketSubmit = (e: React.FormEvent) => {
    ...
    setTicketSent(true)
    toast({ title: "Support ticket submitted", description: "We'll reply within 2 hours.", variant: "success" })
  }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-8 max-w-[1000px] mx-auto">
                      ...
                      {group.shortcuts.map((sc, i) => (
                        <div key={i} className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-0.5">
                            {sc.keys.map((k, j) => <ShortcutKey key={j} k={k} />)}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {visibleChangelog.map((item, i) => (
                  ...
                  >
                    <div className="flex-shrink-0 pt-0.5">
                      <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full capitalize", TAG_COLORS[item.tag])}>
                        {item.tag}
                      </span>
                    </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {ARTICLE_CATEGORIES.map((cat, i) => (
                <motion.button
                  ...
                  className={cn(
                    "rounded-2xl border p-3.5 text-left transition-all hover:border-foreground/15",
                    activeCategory === cat.id
                      ? `${cat.bg} ${cat.color.replace("text-", "border-")}`
                ...
                >
                  <div className="rounded-2xl border border-border bg-card p-4 flex items-center gap-3">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Full article library for <strong className="text-foreground">{ARTICLE_CATEGORIES.find(c => c.id === activeCategory)?.label}</strong> coming soon.
                      For now, use the FAQ below or contact support.
                    </p>
                    <button onClick={() => setActiveCategory(null)} className="ml-auto text-muted-foreground hover:text-foreground flex-shrink-0">
                      <X className="h-3.5 w-3.5" />
                    </button>
                  </div>
                </motion.div>
      ...
      {!search && (
        <section>
          <h2 className="font-display font-bold text-base mb-4 flex items-center gap-2">
            <Send className="h-4 w-4 text-brand-indigo" />
                ...
                <motion.form key="form" onSubmit={handleTicketSubmit} className="space-y-3.5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        value={ticketName} onChange={e => setTicketName(e.target.value)} required
                    ...
                    <div>
                      ...
                      <input
                        value={ticketEmail} onChange={e => setTicketEmail(e.target.value)} required type="email"
                  ...
                  <div>
                    ...
                    <textarea
                      value={ticketBody} onChange={e => setTicketBody(e.target.value)} required rows={4}
