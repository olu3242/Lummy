...
export class AiRunService {
  ...
  async execute(input: { tenantId: string; runId: string; promptKey: string; promptVersion: number; provider: string; fallbackProvider?: string; maxTokens: number }) {
    ...
    const result = await this.providers.execute(input.provider, input.fallbackProvider || null, { tenantId: input.tenantId, prompt: template.content, model: "default", maxTokens: input.maxTokens })
