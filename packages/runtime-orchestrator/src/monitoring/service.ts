...
export class WorkerMonitorService {
  ...
  emitQueueSnapshot(queue: string, queueDepth: number, reason: string) {
    this.traces.push({ jobId: `queue:${queue}`, queue, event: "job.started", at: new Date().toISOString(), details: { queueDepth, reason } })
  }
