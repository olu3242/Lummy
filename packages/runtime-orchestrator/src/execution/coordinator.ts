...
export class ExecutionCoordinator {
  constructor(private readonly queue: QueueService, private readonly lock: LockService, private readonly monitor: WorkerMonitorService, private readonly scheduler = new SchedulerService(), private readonly retries = new RetryService(), private readonly failures?: FailureInjectionService, private readonly replayValidator = new ReplayIntegrityValidator()) {}
  ...
  async tick(queueName: QueueName, handler: QueueHandler | undefined) {
    ...
    try {
      ...
      this.monitor.emit({ jobId: job.jobId, queue: queueName, event: "job.started", at: new Date().toISOString(), details: { correlationId: job.correlationId, attempt: job.attempt, maxAttempts: job.maxAttempts } })
      logRuntime("job.started", { queue: queueName, jobId: job.jobId, correlationId: job.correlationId, attempt: job.attempt, replayCount: job.replayCount || 0 })
      try {
        const res = await handler(job)
        if (!res.ok) throw new Error(res.error || "handler returned not ok")
        this.monitor.emit({ jobId: job.jobId, queue: queueName, event: "job.completed", at: new Date().toISOString(), details: { correlationId: job.correlationId, attempt: job.attempt, latencyMs: Date.now() - startedAtMs } })
        logRuntime("job.completed", { queue: queueName, jobId: job.jobId, correlationId: job.correlationId, attempt: job.attempt, latencyMs: Date.now() - startedAtMs })
      } catch (error) {
        const nextAttempt = job.attempt + 1
        if (nextAttempt < job.maxAttempts && RETRY_QUEUE[queueName]) {
          const backoffMs = this.retries.nextBackoffMs(nextAttempt)
          await this.queue.enqueue(RETRY_QUEUE[queueName]!, { ...job, queue: RETRY_QUEUE[queueName]!, attempt: nextAttempt, runAt: new Date(Date.now() + backoffMs).toISOString(), payload: { ...job.payload, retry: { lastError: `${error}`, at: new Date().toISOString(), backoffMs } } })
          this.monitor.emit({ jobId: job.jobId, queue: queueName, event: "job.retried", at: new Date().toISOString(), details: { correlationId: job.correlationId, nextAttempt, backoffMs, retryQueue: RETRY_QUEUE[queueName] } })
          return
        }
        if (DLQ_QUEUE[queueName]) await this.queue.enqueue(DLQ_QUEUE[queueName]!, { ...job, queue: DLQ_QUEUE[queueName]!, payload: { ...job.payload, failure: { error: `${error}`, at: new Date().toISOString(), queue: queueName, correlationId: job.correlationId } } })
        this.monitor.emit({ jobId: job.jobId, queue: queueName, event: "job.dead_lettered", at: new Date().toISOString(), details: { correlationId: job.correlationId, attemptsExhausted: nextAttempt >= job.maxAttempts, dlqQueue: DLQ_QUEUE[queueName], error: `${error}` } })
        logRuntime("job.dead_lettered", { queue: queueName, jobId: job.jobId, correlationId: job.correlationId, attempt: nextAttempt, error: `${error}` })
      }
