...
import { createPaystackCheckoutSession } from '../../../src/lib/payments/paystack/provider'
...
export class PaystackProvider implements PaymentProviderInterface {
  async initializePayment(input: InitializePaymentInput): Promise<ProviderInitializeResult> {
    const res = await createPaystackCheckoutSession({
      ...
      customerEmail: input.customerEmail || String(input.metadata?.customerEmail || ''),
      metadata: input.metadata || {},
      successUrl: input.successUrl || '',
      cancelUrl: input.cancelUrl || '',
    })
    return { providerReference: res.providerReference, checkoutUrl: res.checkoutUrl, status: 'initiated', raw: { provider: 'paystack' } }
  }
