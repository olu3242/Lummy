import type { InitializePaymentInput, PaymentStatus, ProviderInitializeResult, NormalizedTransaction } from './provider-types'
...
export async function createPaymentSession(db: DatabaseClient, providerName: string, input: InitializePaymentInput, correlationId?: string) : Promise<ProviderInitializeResult> {
  ...
  const existingResult = await db.findOne<PaymentTransactionLog>('payment_transaction_logs', { idempotency_key: idempotencyKey })
  ...
  try {
    ...
    await db.update('payment_transaction_logs', { idempotency_key: idempotencyKey }, { provider_reference: res.providerReference, checkout_url: res.checkoutUrl, provider_payload: res.raw || {}, status: res.status || 'initiated', updated_at: new Date().toISOString() })
    ...
    await db.update('payment_transaction_logs', { idempotency_key: idempotencyKey }, { status: 'failed', provider_payload: { error: err instanceof Error ? err.message : String(err) }, updated_at: new Date().toISOString() })
...
export async function handleProviderWebhook(db: DatabaseClient, providerName: string, headers: Record<string,string>, rawBody: string, correlationId?: string): Promise<NormalizedTransaction | null> {
  ...
  const existingResult = await db.findOne<PaymentTransactionLog>('payment_transaction_logs', { provider_reference: tx.providerReference })
  const existing = existingResult.data
  if (existing) {
    ...
    // otherwise update
    await db.update('payment_transaction_logs', { provider_reference: tx.providerReference }, { status: tx.status, provider_payload: tx, updated_at: now })
